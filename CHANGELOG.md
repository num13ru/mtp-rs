...
- **[lib] `Storage::download_windowed(handle, window_size)`: read a large file without monopolizing the PTP session.** MTP allows one PTP session per device, and a streaming `download_stream` holds it open for the *whole* file, so the device can't service a folder listing or navigation until the read finishes or is aborted (and aborting a multi-GB in-flight read costs ~35s to drain the USB backlog). `download_windowed` returns a `WindowedDownload` whose `next_window()` reads the file as a sequence of small bounded `GetPartialObject64` transactions, each of which **releases the session on return**, so a consumer can interleave other device work (a listing, a cancel-flag check) *between* windows, and it just works at its natural cost. `WindowedDownload` owns the bookkeeping (total size, offset, window sizing, EOF) but no policy: the consumer interposes its own logic between `next_window()` calls: that boundary is the design. Nothing is held between windows, so to stop early you just drop it (no `cancel()` needed, unlike `FileDownload`). Edge cases mirror `download_stream_from_offset`: empty file / `offset == size` ⇒ first `next_window()` is `None` (no read issued); `offset > size` ⇒ `Error::InvalidData` before any USB I/O; a 0-byte read while bytes remain ⇒ `Error::InvalidData` (a device stall, not a silent EOF or a spin); a short non-zero mid-file read advances by the bytes actually returned. `size()` reports the full object size so progress/ETA stays anchored. Companions: `download_windowed_from_offset(handle, offset, window_size)` (resumable), `download_windowed_default(handle)`, and `DEFAULT_DOWNLOAD_WINDOW` (8 MiB ≈ ~80ms/window on a Pixel 9 Pro XL: a documented suggestion, `window_size` stays an open parameter). New `examples/windowed_download.rs` reads a file window-by-window while listing a folder between windows, against the virtual device (no hardware needed). The `download_stream` / `download_stream_from_offset` docs gained a "⚠️ Session monopoly" note pointing here. Requires the device to advertise `GetPartialObject64` (`0x95C1`); most modern Android devices do.
...
- **[cli] Bump `mtp-rs` dependency to 0.22.0.** No functional CLI change; tracks the library release.
...
- **[lib] `Storage::download_stream_from_offset(handle, offset)` — resumable streaming downloads.** A streaming download (true USB streaming, same machinery as `download_stream`) that starts at a byte offset and streams `[offset, size)` to EOF, via `GetPartialObject64` (64-bit offset, so files larger than 4 GB resume correctly). The point is releasing the session: MTP is one PTP session per device, and an in-flight `GetObject` owns it until it completes or aborts, so a host can't list folders while a download is open. A consumer can now `cancel()` an in-flight download (which drains the pipe and frees the session, so navigation works again), remember how many bytes it kept, and reopen from exactly that offset to fetch the rest. Offset semantics: `offset == 0` equals a full download; `offset == size` is a clean empty-stream EOF (resuming an already-complete file is a no-op, not an error); `offset > size` returns `Error::InvalidData` immediately, before any USB I/O, so it never hangs. The returned `FileDownload` reports the full object `size()` (not the segment length) so progress/ETA stay anchored across a resume. A single call requests at most `u32::MAX` (~4 GiB) bytes (`GetPartialObject64`'s `max_bytes` is 32-bit); a larger tail is fetched across multiple resumes. New `examples/resume_download.rs` demonstrates the half-download → pause → resume → verify cycle against the virtual device (no hardware needed).
...
- **[lib] `rekey_virtual_object(serial, rel_path)` (feature `virtual-device`).** Reassigns a tracked object's handle while leaving the object and its on-disk contents in place: the old handle starts returning `InvalidObjectHandle`/`InvalidParentObject` and a fresh listing of the parent surfaces the new handle (direct children are re-parented so `GetObjectInfo` keeps reporting a live parent). This reproduces Android MediaProvider's habit of re-keying object IDs across a media rescan, which silently invalidates a handle a host cached when it last listed a folder — the exact precondition for a stale-handle upload/delete failure that the stable-handle virtual device couldn't previously produce. Queues no events (it models the device moving on before the host has observed the change), so consumers can drive their stale-handle recovery paths end-to-end against the virtual device instead of poking internal caches.

### Fixed

- **[lib] Sessions self-heal after a transaction is abandoned mid-flight, instead of dying permanently.** A PTP transaction is command → (data) → response over one bulk pipe. If an operation's future is dropped after its command goes out but before the response is drained — a superseded directory listing, a cancelled task, a `timeout` that races the future — the device's reply stays in the pipe, so every later operation reads the *previous* reply and the transaction-ID stream desyncs by one ("Transaction ID mismatch: expected N, got N-1"), permanently. Observed in the field: browsing a Xiaomi's `gallery_disk_cache` (thousands of tiny files, ~24 s listings) with concurrent prefetch listings reliably dropped one mid-`GetObjectInfo`, wedging the session so the consumer fell back and the device looked like it disconnected. `PtpSession` now arms each operation across its command→response cycle (`execute`, `execute_with_receive`, `execute_with_send`, the streaming send, and `ReceiveStream`); any non-clean exit flags shared recovery state, and the next operation drains the bulk pipe (via the validated `cancel_transfer` CLASS_CANCEL + read-until-idle path) before sending. Recovery is lazy because `Drop` can't run an async drain in a runtime-agnostic crate. No API change: consumers get this for free. Distinct from the existing `reset_device()`, which recovers cross-process poison (a host that died mid-transfer) where no live session exists to self-heal.
